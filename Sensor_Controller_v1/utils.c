...
/* Print int in hex
 * value: 16-bit value to print
 * buf: character buf of at least 6 bytes
 * returns number of bytes to print
 */
uint8_t print_int(uint16_t value, uint8_t *buf){
	//Current
	buf[0] = '0';
	buf[1] = 'x';
	hex2ascii_int(value, &buf[2], &buf[3], &buf[4], &buf[5]);
	return 6;
}
